<?php 
/**
 * Parse the slide array
 * Gets array of theme mod slides, makes sure each item has an image, no blanks
 */
...
function wws_customize_register_homebox( $wp_customize ) {
	/*** Add boxes to home page ***/
	//See: http://wordpress.stackexchange.com/questions/144004/get-attachment-id-from-wp-customizer
	//Should save post ID rather than URL in theme mods
	$NumBoxes = 3;
...
function wwws_homebox_customize_register( WP_Customize_Manager $wp_customize ) {
	$wp_customize->selective_refresh->add_partial( 'wws_options[homebox][0][image]', // settings name
		array(
...
add_action( 'customize_register', 'wwws_homebox_customize_register' );
